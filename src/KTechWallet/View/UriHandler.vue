...
export default class UriHandler extends Vue {
  ...
  created() {
    ...
    if (!uri.startsWith("web+pasc:")) {
      this.error = "start != web+pasc:";
      return;
    }

    uri = uri.slice(9, uri.length);
    let split = uri.split("&");
    ...
    if (this.uriParameterMap.get("action") === "pay") {
      ...
      if (this.uriParameterMap.get("payloaddisplay")) {
        data += "payloaddisplay=" + this.uriParameterMap.get("payloaddisplay") + "&";
      }
