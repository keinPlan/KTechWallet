...
import { Hash, Kdf, eKeyTypes, EcCrypto, Aes } from "@/KTechLib/KtlCrypto";
...
export default class Tools extends Vue {
    ...
    var gg = PascalPublicKey.CreateFromPrivateKey(eKeyTypes.SECP256K1, pk);
  ...
  async test3() {
    ...
    var gg = PascalPublicKey.CreateFromPrivateKey(eKeyTypes.SECP521R1, pk);

    let data = new Uint8Array(32).fill(0);

    console.log(pk);

    let sig = EcCrypto.sign(eKeyTypes.SECP521R1, pk, data);

    console.log(sig);
    console.log(EcCrypto.CheckSig(eKeyTypes.SECP521R1, pk, data, sig.r, sig.s));
  }
